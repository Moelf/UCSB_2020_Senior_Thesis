...
jets are \cc{produced} identified using the DeepFlavour algorithm.\cite{deepflavour} \add{These are the jets
...
\add{First, a veto on the b-tagged jet is placed to reduce the background events originated from a pair
of top quarts ($t\bar{t}$) since top quark always decays to a bottom quark. This cut also doesn't
reduce signal efficiency at all.} Also, many of the cuts are related to angles between various physical objects presented in the
...
are present in the underlying analysis framework to \cc{reject events with pathological reconstruction.}
